...
let chart4 = new ApexCharts(document.querySelector("#trend_graph"), {
    chart: {
        type: 'line',
        height: 350
    },
...
Livewire.on('refreshCharts', () => {
    // Get raw data from the event detail
    const rawData = event.detail.raw_data;
    const trend_line =event.detail.trend_line;
    console.log(rawData, trend_line)
    // Convert raw data into a format suitable for ApexCharts
    const cpuTemperatureData = rawData.map(item => ({
        x: new Date(item.created_at).getTime(),
        y: parseFloat(item.util)
    }));
